  ...
  describe("when the voter has voting power", async function () {
    ...
    describe("when the voter interact with the delegate function in the contract", function () {
      ...
      it("triggers the Delegated event", async function () {
        ...
        await expect(ballotContract.connect(accounts[1]).delegate(delegateAddress))
          .to.emit(ballotContract, "Delegated")
          .withArgs(accounts[1].address, delegateAddress, 2, false, 0, 0);
      });
    });

    describe("when the voter interact with the delegate function in the contract, and the delegate has other delegate", function () {
      // TODO
      it("triggers the Delegated event where 'to' address has delegate", async function () {
        const delegateAddress = accounts[3].address;
        await giveRightToVote(ballotContract, delegateAddress);
        const toAddress = accounts[2].address;
        await giveRightToVote(ballotContract, toAddress);
        await ballotContract.connect(accounts[2]).delegate(delegateAddress);
        await expect(ballotContract.connect(accounts[1]).delegate(toAddress))
          .to.emit(ballotContract, "Delegated")
          .withArgs(accounts[1].address, delegateAddress, 3, false, 0, 0)
      });
    });

    describe("when the voter interact with the delegate function in the contract, and the delegate has already voted", function () {
      // TODO
      it("triggers the Delegated event where 'to' address has voted", async function () {
        const toAddress = accounts[2].address;
        await giveRightToVote(ballotContract, toAddress);
        await ballotContract.connect(accounts[2]).vote(1);
        await expect(ballotContract.connect(accounts[1]).delegate(toAddress))
          .to.emit(ballotContract, "Delegated")
          .withArgs(accounts[1].address, toAddress, 0, true, 1, 2)
      });
    });
    ...
    describe("when the voter interact with the delegate function in the contract, and the delegate has other delegate that has already voted", function () {
      // TODO
      it("triggers the Delegated event where the delegate has other delegate that has already voted", async function () {
        const toAddress = accounts[2].address;
        const delegateAddress = accounts[3].address;
        await giveRightToVote(ballotContract, toAddress);
        await giveRightToVote(ballotContract, delegateAddress);
        ...
        await expect(ballotContract.connect(accounts[1]).delegate(toAddress))
          .to.emit(ballotContract, "Delegated")
          .withArgs(accounts[1].address, delegateAddress, 0, true, 1, 3)
      });
    });
  });
